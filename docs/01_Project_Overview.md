...
여러 사용자가 동일한 운동기구를 동시에 예약하는 상황에서 중복 예약이 발생하지 않도록 Redis Distributed Lock을 이용하여 예약의 원자성을 보장한다.
...
예약 후 일정 시간 내 체크인하지 않으면 예약을 자동 취소하여 리소스 회전율을 높인다.
...
- 예약 시작 전후 허용 시간 내에서만 체크인 가능하다.
- 체크인하지 않으면 예약은 자동 취소된다.

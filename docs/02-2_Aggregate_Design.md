...
- Resource는 대표 이미지(S3)가 실제로 어디에 저장되는지 알지 못한다. Entity는 `imageKey` 상태(nullable String)만 `changeImageKey()`/`removeImageKey()`로 관리하며, S3 SDK/Bucket 이름/Presigned URL 생성은 `ResourceImageStorage` 포트(`domain/resource/domain/storage`)와 그 구현체 `S3ResourceImageStorage`(`global/infrastructure/s3`)로 분리되어 있다. MySQL(`Resource.imageKey`)과 S3 Object는 하나의 ACID Transaction으로 묶이지 않으므로, `AdminResourceImageService`는 다음 순서로 정합성을 맞춘다: (1) 업로드/교체는 새 Object를 먼저 올린 뒤(Fail-Closed: 업로드 실패 시 `imageKey`를 변경하지 않고 API가 실패한다) DB를 갱신하고, `TransactionSynchronization.afterCompletion()`에서 COMMIT이면 옛 Object를 지우고 ROLLBACK이면 방금 올린 새 Object를 보상 삭제한다. (2) 삭제는 DB의 `imageKey`를 먼저 비우고 COMMIT한 뒤에만 S3 Object를 지운다(반대 순서로 하면 "DB에는 없는데 실제 Object만 남는" 것과 반대로 "DB는 비었는데 실제 Object가 남는" 상황이 생길 수 있으나, 이 순서를 지키면 최소한 "DB에는 남아있는데 실제 Object가 없는" broken reference는 발생하지 않는다). COMMIT 이후에 수행되는 S3 cleanup(옛 Object/rollback 보상 Object 삭제) 실패는 Fail-Open으로 처리한다 — 이미 COMMIT된 DB 상태를 되돌릴 방법이 없고 이후 조회는 항상 최신 `imageKey` 기준으로 정상 동작하므로, WARN 로그만 남기고 stale S3 Object가 orphan으로 남을 수 있음을 감수한다. 삭제 API는 `imageKey`가 이미 `null`이어도 204로 멱등하게 성공한다.
- 조회 Response의 `imageUrl`은 `imageKey`로부터 매 요청마다 새로 생성하는 1시간짜리 Presigned GET URL이며 DB/Redis 어디에도 영구 저장하지 않는다. `ResourceService.getResourceDetail()`이 사용하는 `ResourceCacheRepository`는 이미 resolve된 `imageUrl`을 포함한 `ResourceResponse` 전체를 캐시하는데, 이는 Resource Cache TTL(`Duration.ofMinutes(10)`)이 Presigned URL 유효기간(`Duration.ofHours(1)`)보다 항상 짧게 유지되는 한 캐시가 만료된 URL을 반환할 수 없기 때문에 안전하다(캐시 자체가 URL보다 먼저 사라진다). 이 두 값 중 하나라도 바뀌어 TTL ≥ 유효기간이 되면 캐시된 imageUrl이 만료된 채로 노출될 수 있으므로 반드시 함께 재검토해야 한다.
